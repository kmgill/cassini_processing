    ...
    for file_name in source:
        if file_name[-3:].upper() != "CUB":
            ...
            if map is None:
                ...
                if not rings:
                    if not reprojecting:
                        cameras.cam2map(file_name, out_file, projection=projection, band=band)
                    else:
                        cameras.map2map(file_name, out_file, projection=projection, band=band)
                else:
                    cameras.ringscam2map(file_name, out_file, projection=projection, band=band)
            else:
                out_file = "%s_%s.cub" % (file_name[0:-4], "reprojected")
                if not rings:
                    if not reprojecting:
                        cameras.cam2map(file_name, out_file, map=map, resolution="MAP", band=band)
                    else:
                        cameras.map2map(file_name, out_file, map=map, resolution="MAP", band=band)
                else:
                    cameras.ringscam2map(file_name, out_file, map=map, resolution="MAP", band=band)
